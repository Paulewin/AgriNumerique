...
```{r, echo=FALSE}
library(ggplot2)
ggplot(simulation, aes(x = possession_telephone, fill = possession_telephone)) + geom_bar(position = "dodge") + labs(title = "Histogramme de possession d'un téléphone", x = "Possède un téléphone", y = "Proportion", fill = "Accès à l'info") + theme_minimal()
```
...
```
Il y  a de différence statistique dans l'utilisation des outils numériques à savoir: SMS alete, réseaux sociaux en fonction de la culture pratiquée. 
En somme, ls test de dépendance de la possession et de l'accès au telephone n'est pas significatif  alors que l'usage des outls numériques l'est. Cela veut dire que la barrière est l'accès (posséder le téléphone).
...
* Le connecté * (niveau d'étude supérieur, agé de 25à 40ans et utilisant régulièrement les outils numériques)
* L'intermédiaire* (niveau d'étude primaire, agé de 25 à 40 ans et utilisant les outils numériques )
* Le non connecté* (Aucun niveau d'étude, agé de 41 à 60 ans n'utilisant pas les outils numériques)
