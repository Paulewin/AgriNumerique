...
table_croise <- table(simulation$commune, simulation$superficie_total_finale)
test_fisher2 <- fisher.test(table_croise, simulate.p.value = TRUE)
print(test_fisher2)
```
...
ggplot(simulation, aes(x = sexe, y = superficie_total_finale, fill = sexe)) +
  geom_boxplot() +
  labs(title = "Graphe A2: Superficie exploitée selon le sexe")
...
ggplot(simulation, aes(x = etude, fill = type_agri)) +
  ...
  labs(
    title = "Graphe A1: Répartition du type d'agriculture selon le niveau d'étude",
    x = "Niveau d'étude",
    y = "Proportion (%)",
...
fviz_mca_var(res.mca, 
             ...
             title = "Graphe A3: Analyse en composante multiple",
...
L'analyse statistique, bien que révélant une certaine homogénéité des pratiques à travers les tests de Fisher, souligne par l'ACM une structuration de l'espace agricole où le capital scolaire commence à orienter les choix des techniques de production. Ces résultats prouvent que les systèmes de production ne sont pas dictés essentiellement par le profil social, mais restent ouverts à une transformation par l'outil numérique. Dès lors, la solution proposée doit offrir les mêmes fonctionnalités mais être inclusive dans son ergonomie pour une adption généralisée des meilleures pratiques agricoles.