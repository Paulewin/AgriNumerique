...
```{r}
library(readr)
Agri_Numérique <- read_csv("Agri Numérique_  VF.csv")
View(Agri_Numérique)
```
...
ggplot(frequences, aes(x = Effet, y = Pourcentage)) +
  ...
  labs(title = "Graphe B2: Pourcentage de producteurs par effet climatique",
       x = "Effet",
       y = "Pourcentage (%)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
moyen_gestion_risques  <- simulation$moyen_gestion_risques
...
```{r}
gestion_risques_df  <- simulation[, c("moyen1", "Moyen2", "Moyen3")]
...
barplot(table(adaptation_binaire$niveau_adaptation),
        ...
        main = "Graphe B3: Niveaux d'adaptation des producteurs",
...
ggplot(data_long, aes(x=categorie, y=indice_adaptation, fill=categorie)) +
  ...
  labs(title="Graphe B4: Indice moyen d’adaptation selon les caractéristiques socio-économiques",
...
ggplot(coef_df, aes(x = reorder(term, estimate), y = estimate, fill = term)) +
  ...
  coord_flip() +
  labs(title = "Graphe B5: Effet des variables sur l'indice d'adaptation",
